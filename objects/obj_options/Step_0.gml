if (buffer == 0)
{
    ...
    var q = opt[selected];
    var move = (q.type != states.bounce) ? (key_left2 + key_right2) : (key_left + key_right);
    var move2 = -key_up2 + key_down2;
    
    if (move != 0 && q.type != states.walljump && q.type != states.hurt)
    {
        q.val += move;
    ...
    if (q.type != states.hurt)
    {
        if (key_jump2)
        {
            if (q.type == states.capepound)
            {
                q.val += 1;
