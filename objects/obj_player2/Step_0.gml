...
if (hitstun == 0)
{
    ...
    if (_enemy != -4)
    {
        if (point_distance(x, y, _enemy.x, _enemy.y) < 640)
            chasing_enemy = true;
        else
            chasing_enemy = false;
    }
    ...
    else
    {
        ...
        if (point_distance(x, y, _enemy.x, _enemy.y) < 224)
            key_down2 = true;
        else
            key_down2 = false;
    ...
    if (((obj_player1.y < y && point_distance(0, y, 0, obj_player1.y) > 128) || (place_meeting(x + xscale, y - 1, obj_solid) && !place_meeting(x + 32, y - 400, obj_solid)) || (state == states.wallslide && vsp > 0 && !place_meeting(x, y - 128, obj_solid) && obj_player1.y < y)) && !place_meeting(x, y - 128, obj_solid) && amt_jumps < 6)
    {
        key_jump = true;
        jumptim = 20;
    ...
    if (state != states.capepound && y < obj_player1.y && point_distance(0, y, 0, obj_player1.y) > 1024 && !grounded)
        key_jump2 = true;
...
if (state == states.fork || (state == states.running && char != "N" && movespeed > speeds[1]) || (state == states.cape && movespeed > speeds[1]) || state == states.capepound)
    killmove = true;
else
    killmove = false;
